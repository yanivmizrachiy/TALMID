# RULES — אתר חדש (מיקרו־אתר סטטי מבודד)

## כלל ברזל — אין בלבולים
- כל קבצי הדף החדש נמצאים **רק** תחת התיקייה: `אתר חדש/`.
- אין תלות (HTML/CSS/JS/Backend) בשום קובץ קיים במערכת המקורית.
- `data/data.json` של האתר החדש מכיל **שמות בלבד** (תלמידים/מורים/הקבצות) ומספרים מחושבים — בלי ציונים, בלי הערות, בלי מזהים.

## העתק מלא של הדרישות המקוריות (כפי שנמסרו)
...
## עדכון מהמשתמש (למניעת בלבול)
- "תבצע את המשימה באופן החכם ביותר והמדוייק ביותר האפשרי לאתר החדש תשיםן את המידע בתוך תיקיית קבצים בשם אתר חדש כדי שלא יהיה בלבול עם שאר הפרוייקט"

## יישום בפועל
- האתר החדש נוצר תחת `אתר חדש/` בלבד.
- הנתונים נטענים מקובצי CSV קיימים בפרויקט, אך נשמרים באתר החדש כשמות בלבד (string) תחת `students[]`.
